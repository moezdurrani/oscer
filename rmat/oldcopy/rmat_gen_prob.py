...
def split_cores(rem_cores, a_n, b_n, c_n, d_n):
    ...
    exact = [p * rem_cores for p in probs]
    splits = [int(e) for e in exact]
    total_used = sum(splits)
    remaining = rem_cores - total_used

    # Sort by highest probability (your intent), assign remaining one by one
...
def generate(a, b, c, d, k, s, seed_val, output_dir):
    ...
    exp_edges = tot ** k
    size = 2 ** k
    # cpu count or total number of parallel processes
