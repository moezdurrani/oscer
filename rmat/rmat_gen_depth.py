...
def rec_gen_parallel(exp_edges, k, u_offset, v_offset, size, a_n, b_n, c_n, d_n, depth, output_dir):
    ...
    # depth_limit = math.floor(math.log(os.cpu_count(), 4))
    if depth < 3:
...
def generate(a, b, c, d, k, s, seed_val, output_dir):
    ...
    rec_gen_parallel(exp_edges, k, 0, 0, size, a_n, b_n, c_n, d_n, depth, temp_dir)
