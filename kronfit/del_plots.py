...
# Add a small padding so points at the boundary aren't clipped
...
for dataset, k_dict in all_l2_values.items():
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUTPUT_DIR, f"{dataset}_l2_norm_vs_k.png"), dpi=150)
    plt.close(fig)
    ...
    # Individual point overlay with jitter
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(OUTPUT_DIR, f"{dataset}_L2_violin_plot_vs_k.png"), dpi=150)
    plt.close(fig)
