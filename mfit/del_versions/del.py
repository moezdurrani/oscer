...
class mfit:
    ...
    def _get_scaled_initial_matrix(self, init_matrix):
        ...
        if self.n_edges > 0 and self.k > 0:
            # Correct Kronecker expected edges: sum of all n^2 entry products
            # For 2x2 matrix, expected edges = (a+b+c+d)^k / correction
            # Simplest correct approach: scale so sum^k matches n_edges
            # where sum is over the matrix (this matches kronfit's SetForEdges)
            current_sum = np.sum(p_np)
            target_sum = self.n_edges ** (1.0 / self.k)
            scale = target_sum / current_sum
            p_np = p_np * scale
            p_np = np.clip(p_np, 1e-6, 0.999)
        return p_np
    ...
    def _delta_ll_for_swap(self, u, v):
        # --- Edge delta (same as before, but don't commit) ---
        ...
        old_edge_ll = self.edge_ll[affected_edge_idx]
        ...
        edge_delta = new_edge_ll.sum() - old_edge_ll.sum()

        # --- Non-edge delta ---
        ne = self.sampled_non_edges
        mask = (ne[:, 0] == u) | (ne[:, 1] == u) | (ne[:, 0] == v) | (ne[:, 1] == v)
        affected_ne_idx = mask.nonzero(as_tuple=True)[0]
        ne_delta = torch.tensor(0.0, device=self.device)
        new_ne_ll = None
        if len(affected_ne_idx) > 0:
            affected_ne = ne[affected_ne_idx]
            old_ne_ll = self.non_edge_ll[affected_ne_idx]
            perm_u_ne = self.inverse_perm[affected_ne[:, 0]]
            perm_v_ne = self.inverse_perm[affected_ne[:, 1]]
            new_ne_probs = self._get_kron_edge_prob_vectorized(perm_u_ne, perm_v_ne)
            scale = self.num_non_edges / len(ne)
            new_ne_ll = torch.log((1 - new_ne_probs).clamp(min=1e-10)) * scale
            ne_delta = new_ne_ll.sum() - old_ne_ll.sum()

        return (edge_delta + ne_delta,
                affected_edge_idx, new_edge_ll,
                affected_ne_idx if len(affected_ne_idx) > 0 else None, new_ne_ll)
    ...
    def _mcmc_step_for_permutation(self):
        ...
        delta, affected_edge_idx, new_edge_ll, affected_ne_idx, new_ne_ll = self._delta_ll_for_swap(pi, pj)

        if delta > 0 or torch.rand(1).item() < torch.exp(delta).item():
            # Commit both caches on accept
            self.edge_ll[affected_edge_idx] = new_edge_ll
            if affected_ne_idx is not None and new_ne_ll is not None:
