...
class mfit:
    def __init__(self, graph_temp, init_matrix, learning_rate, warmup_mcmc, grad_samples, iterations, device=None):
        ...
        # RMSprop optimizer
        # alpha (0.99): smoothing constant for the running average of squared gradients
        # momentum (0.0): optional momentum term; set >0 (e.g. 0.9) if convergence is slow
    ...
    def fit(self, iterations, grad_samples, warmup_mcmc, mcmc_per_iter):
        ...
        print(f"\nMain Optimization ({iterations} iterations) — RMSprop")
        best_ll = float('-inf')
