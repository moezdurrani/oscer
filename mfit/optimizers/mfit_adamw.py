...
class mfit:
    def __init__(self, graph_temp, init_matrix, learning_rate, warmup_mcmc, grad_samples, iterations, device=None):
        ...
        # AdamW optimizer
        # weight_decay: L2 penalty applied directly to P (not via gradient).
        #   This discourages entries from drifting toward the clamp boundaries (0 or 1).
        #   Try values between 1e-5 and 1e-3. Set to 0.0 to match vanilla Adam exactly.
        # betas: (0.9, 0.999) are PyTorch defaults — rarely need changing.
    ...
    def fit(self, iterations, grad_samples, warmup_mcmc, mcmc_per_iter):
        ...
        print(f"\nMain Optimization ({iterations} iterations) — AdamW")
        best_ll = float('-inf')
